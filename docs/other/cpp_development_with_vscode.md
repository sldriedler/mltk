...
This describes how to development C++ applications in the MLTK using [Visual Studio Code](https://code.visualstudio.com).  
Using this guide, you can build C++ applications for Windows/Linux or supported embedded platforms.


All of the C++ source code may be found at: [__mltk repo__/cpp](../cpp)


Several applications come with the MLTK, these include:  
- [audio_classifier](../cpp/shared/apps/audio_classifier) - Classify real-time microphone audio
- [hello_world](../cpp/shared/apps/hello_world) - Print "hello world" to console
- [model_profiler](../cpp/shared/apps/model_profiler) - Profile an ML model using [Tensorflow-Lite Micro](https://github.com/tensorflow/tflite-micro)


## Install Tools

Before we can install the MLTK, we need to install a few tools first:
...
### 2) Run the MLTK install script

```shell
# Navigate to the mltk repo
cd mltk
...
### 3) Select the build type and build target
...
This creates a CMake variable and tells the build system to use the `brd2601a` platform.  
