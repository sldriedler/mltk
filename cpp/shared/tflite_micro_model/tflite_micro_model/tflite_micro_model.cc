...
)
{
    ...
    if(_accelerator != nullptr)
    {
        _model_details._accelerator = _accelerator->name;
#ifdef TFLITE_MICRO_SIMULATOR_ENABLED
        _accelerator->set_simulator_memory("sram", runtime_buffer, _model_details._runtime_memory_size);
...
bool TfliteMicroModel::invoke() const
{
    ...
    if(_accelerator != nullptr)
    {
        retval = _accelerator->invoke_simulator([this]() -> bool
        {
