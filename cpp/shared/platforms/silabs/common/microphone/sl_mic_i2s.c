...
sl_status_t sl_mic_start_streaming(void *buffer, uint32_t n_frames, sl_mic_buffer_ready_callback_t callback)
{
  ...
  // This effectively determines the resolution of sl_mic_get_timestamp()
  ...
  mic_context.timestamp_ms = 0;
  mic_context.ms_per_callback = ms_per_callback;
 ...
 ******************************************************************************/
uint32_t sl_mic_get_timestamp()
{
  return mic_context.timestamp_ms;
}
...
static bool dma_complete(unsigned int channel, unsigned int sequenceNo, void *userParam)
{
  ...
  mic_context.timestamp_ms += mic_context.ms_per_callback; 
  return true;
}
