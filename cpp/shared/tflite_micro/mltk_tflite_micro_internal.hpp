...
{ \
  ...
  if(mltk::_accelerator != nullptr) mltk::_accelerator->start_profiler(-1); \
}

#define STOP_INFERENCE_PROFILER(subgraph_idx) \
if(subgraph_idx == 0) \
{ \
  if(mltk::_accelerator != nullptr) mltk::_accelerator->stop_profiler(-1); \
...
{ \
  ...
  if(mltk::_accelerator != nullptr) mltk::_accelerator->start_op_profiler(op_idx, (mltk::_kernel_profilers != nullptr) ? mltk::_kernel_profilers[op_idx] : nullptr); \
}
...
if(subgraph_idx == 0) \
{ \
  if(mltk::_accelerator != nullptr) mltk::_accelerator->stop_op_profiler(op_idx, (mltk::_kernel_profilers != nullptr) ? mltk::_kernel_profilers[op_idx] : nullptr); \
...
#define START_INFERENCE_PROFILER(subgraph_idx) \
  const int _accelerator_loop_count = (mltk::_accelerator != nullptr) ? mltk::_accelerator->get_profiler_loop_count() : 1; \
  ...
  if(mltk::_accelerator != nullptr) mltk::_accelerator->start_profiler(_accelerator_loop);

#define STOP_INFERENCE_PROFILER(subgraph_idx) \
  if(mltk::_accelerator != nullptr) mltk::_accelerator->stop_profiler(_accelerator_loop); \
...
extern const TfliteMicroAccelerator* _accelerator;
